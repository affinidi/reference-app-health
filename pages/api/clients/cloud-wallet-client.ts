...
export const cloudWalletClient = {
  ...
  getCredentialById: async (input: { id: string }, options: Options): Promise<{ vc: VerifiableCredential }> => {
    const { data: vc } = await axios<VerifiableCredential>(
      `${cloudWalletApiUrl}/v1/wallet/credentials/${input.id}`,
      {
        method: 'GET',
        headers: {
          'Api-Key': issuerApiKeyHash,
